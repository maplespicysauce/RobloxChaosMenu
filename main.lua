--// xAI ULTIMATE CHAOS MENU v7 PRO - FULLY INTEGRATED KEY SYSTEM IN MAIN.LUA \\--
-- 100% CLEAN | Synapse X / Krnl / Fluxus | 2025
-- ONE FILE ONLY: https://raw.githubusercontent.com/maplespicysauce/RobloxChaosMenu/main/main.lua

--// HARD-CODED KEY (OFFICIAL)
...
--// SHOW KEY SYSTEM → ON SUCCESS, LOAD MENU
ShowKeySystem(function()
    --// === FULL MENU BELOW (RUNS ONLY AFTER KEY) ===
    ...
    MainSection:NewButton("Open Fly GUI v3", "Advanced Fly GUI (XNEOFF)", openFlyGui)
    MainSection:NewSlider("Inf Jump Height", 500, 50, 100, function(v) _G.JumpHeight = v end)
    MainSection:NewToggle("Infinite Jump", false, toggleInfJump)

    local VisualTab = Window:NewTab("Visuals & NSFW")
    local VisualSection = VisualTab:NewSection("✦ Chaos Zone ✦")
    VisualSection:NewToggle("Jerk Off LOOP 💦", false, function(s) if s then startJerkLoop() else stopJerkLoop() end end)
    VisualSection:NewToggle("Player ESP", false, toggleESP)

    local GUITab = Window:NewTab("GUIs")
    local GUISection = GUITab:NewSection("External GUIs")
    GUISection:NewButton("Open Fly GUI v3", "Advanced Fly GUI (XNEOFF)", openFlyGui)
    GUISection:NewButton("FE Trolling GUI", "Sky Hub Trolling\nCredits in 3 dots", loadTrollingGUI)
    GUISection:NewButton("Chat Tags Bypass", "Say anything (Axur V2)", loadChatBypass)
    ...
    NDSSection:NewButton("NDS Hub", "Credits to w2pr on ScriptBlox", loadNDSHub)

    local PlayerTab = Window:NewTab("Player")
    local PlayerSection = PlayerTab:NewSection("Stats")
    PlayerSection:NewSlider("WalkSpeed", 1000, 0, 16, updateWalkSpeed)
    PlayerSection:NewSlider("JumpPower", 1000, 0, 50, updateJumpPower)
